...
@META_ARCH_REGISTRY.register()
class SOLOv2_Simplified(SOLOv2):
    def __init__(self, cfg):
        super().__init__(cfg)
    ...
    def forward(self, batched_inputs, input_images=None, mode='mrcnn', item='nucleus', norm=True):
        ...
        features = self.backbone(images) if torch.is_tensor(images) else self.backbone(images.tensor)
        if mode == 'cycle':
            final = self.get_cycle_result(features)
            return final
    ...
    def get_cycle_result(self, features):
        ...
        p2 = F.interpolate(p2, scale_factor=1, mode="bilinear", align_corners=True)
    ...
    def preprocess_image(self, batched_inputs, images=None, mode='A', norm=True):
